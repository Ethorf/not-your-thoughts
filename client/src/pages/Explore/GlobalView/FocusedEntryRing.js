...
const FocusRingMesh = ({ position, size, isFading, onFadeComplete }) => {
  ...
  return (
    <mesh position={position.toArray()} renderOrder={4}>
...
const FocusedEntryRing = ({ entryId, clusterViews, dismissSignal = 0 }) => {
  ...
  const focusedNodeRing = useMemo(() => {
    ...
    for (const view of clusterViews) {
      if (view?.mainNode?.node?.id != null && String(view.mainNode.node.id) === entryIdString) {
        return {
          position: view.mainNode.position,
      ...
      if (firstOrderMatch) {
        ...
        return {
          position: firstOrderMatch.position,
      ...
      if (secondOrderMatch) {
        return {
          position: secondOrderMatch.position,
          size: getGlobalConnectionSphereSize(
  ...
  if (!focusedNodeRing) return null

  return (
    <FocusRingMesh
      position={focusedNodeRing.position}
      size={focusedNodeRing.size}
