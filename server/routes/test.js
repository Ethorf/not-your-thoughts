...
router.get('/user_test', validInfo, async (req, res) => {
  ...
  try {
    const user = await pool.query('SELECT * FROM test_users_2 WHERE email = $1', [userEmail])
...
router.post('/user_test_create', validInfo, async (req, res) => {
  try {
    const user = await pool.query(
      "INSERT INTO test_users (name, email, password) VALUES ('shart', 'shartinthedarky@gmail.com', 'kthl8822');"
    )
